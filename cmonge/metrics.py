...
def average_r2(target: jnp.ndarray, transport: jnp.ndarray) -> float:
    """
    Calculate the correlation coefficient r^2 between the means of
    average features in target and tansport.
    """
...
def drug_signature(target: jnp.ndarray, transport: jnp.ndarray) -> float:
    """Calculates the euclidien distance between the marginal means of
    the target and transported measures."""
...
def compute_scalar_mmd(
    ...
    and the transported measures, using gaussian kernel,
    averaging for different gammas.
    """
